...
int main(int argc, char *argv[])
{
    ...
    {
        SLAM.SaveTrajectoryEuRoC("CameraTrajectory_mono-iner-euroc.txt");
        SLAM.SaveKeyFrameTrajectoryEuRoC("KeyFrameTrajectory_mono-iner-euroc.txt");
    }
    return 0;
}
                ...
                vector<string> &vstrImages, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            double t;
            ss >> t;
            vTimeStamps.push_back(t/1e9); // 时间戳÷1e9存储成double类型，避免越界
        }
    }
}
