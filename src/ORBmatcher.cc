...
namespace ORB_SLAM3
{
                ...
                if(nPredictedLevel != -1){
                    ...
                    for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
                    {
                        ...
                        if(F.mvpMapPoints[idx + F.Nleft])
                            if(F.mvpMapPoints[idx + F.Nleft]->Observations()>0) // 该Map点已被当前帧可视
                                continue;
    ...
    /*
     * @brief 当前帧的Map点投影到邻近帧，在半径范围内匹配ORB点，匹配的ORB点已与物点绑定则根据可视信息替换，没绑定则绑定当前Map点
     * */
    ...
    /*
     * @brief 将vpMapPoints中的地图点投影到pKF，投影-匹配到同名点后，vpMapPoints中的Mapi被对应于pKF中的Mapj代替
     * */
