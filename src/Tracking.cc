...
bool Tracking::ParseIMUParamFile(cv::FileStorage &fSettings)
{
    ...
    mpImuCalib = new IMU::Calib(Tbc,Ng*sf,Na*sf,Ngw/sf,Naw/sf); // 噪声项乘以sqrt(freq), 随机游走除以sqrt(freq)? 这是为何
...
void Tracking::PreintegrateIMU()
{
    ...
    // 3. 过滤IMU观测：根据时间戳剔除不在相邻两帧间的IMU观测
    while(true)
    {
        bool bSleep = false;
        {
            ...
            if(!mlQueueImuData.empty())
            {
                ...
                cout.precision(17);
                if(m->t<mCurrentFrame.mpPrevFrame->mTimeStamp-0.001l) // 跳过小于上一帧时间戳的IMU观测
                {
        ...
        if(bSleep) // Q: 循环处理mlQueueImuData数据，每次循环之间为何还加个等待？ 工程经验？
            usleep(500);
    }
    ...
    // 4. mid-point方法，计算相邻时刻加速度和角速度向量的均值，计算IMU观测间的时间间隔，后边会用均值计算位姿【注意：加速度和角速度均值计算的随机游走b_a， b_g, 在积分函数中考虑】
    for(int i=0; i<n; i++)
    {
        float tstep; // 注意tstep可能<0
        cv::Point3f acc, angVel;
        if((i==0) && (i<(n-1)))
        {
            // 起始需要考虑IMU观测时间戳和关键帧时间戳是否对齐造成的偏差
            float tab = mvImuFromLastFrame[i+1].t-mvImuFromLastFrame[i].t; // 上一帧到当前帧，连续IMU观测间的时间间隔
            float tini = mvImuFromLastFrame[i].t-mCurrentFrame.mpPrevFrame->mTimeStamp; // 当前IMU观测到上一帧时间戳的时间间隔
        ...
        {
            // 最后一个IMU观测需要考虑IMU观测时间戳和关键帧时间戳是否对齐造成的偏差
        ...
        else if((i==0) && (i==(n-1)))
        {
            ...
            tstep = mCurrentFrame.mTimeStamp - mCurrentFrame.mpPrevFrame->mTimeStamp; // 此时IMU预积分的dt为相邻相机帧间的时间间隔
        }

        if (!mpImuPreintegratedFromLastKF)
            cout << "mpImuPreintegratedFromLastKF does not exist" << endl;
        // 两种预积分：1）从上一关键帧；2）从上一邻近帧； 但IMU观测是一致，未区分来自LastKF还是LastFrame
        mpImuPreintegratedFromLastKF->IntegrateNewMeasurement(acc,angVel,tstep); // 从上一个关键帧开始预积分， 注意tsetp可能<0
        pImuPreintegratedFromLastFrame->IntegrateNewMeasurement(acc,angVel,tstep); // 从上一个邻居帧开始预积分
    }
...
bool Tracking::PredictStateIMU()
{
    ...
    {
        // Map被修改且上一关键帧存在，则从上一关键帧开始利用IMU观测推算当前帧的位姿
        ...
        const cv::Mat Gz = (cv::Mat_<float>(3,1) << 0,0,-IMU::GRAVITY_VALUE); // 重力加速度向量(Z轴负方向)
        const float t12 = mpImuPreintegratedFromLastKF->dT;

        // 根据上一关键帧的P, R, V和dR, dP, dV推算当前帧的P, R, V 参考forster论文式（25）
        ...
        mCurrentFrame.SetImuPoseVelocity(Rwb2,twb2,Vwb2); // 更新当前帧的位姿
        mCurrentFrame.mPredRwb = Rwb2.clone();
        mCurrentFrame.mPredtwb = twb2.clone();
        mCurrentFrame.mPredVwb = Vwb2.clone();
        mCurrentFrame.mImuBias = mpLastKeyFrame->GetImuBias(); // 偏置项传递
        mCurrentFrame.mPredBias = mCurrentFrame.mImuBias;
        return true;
    }
    else if(!mbMapUpdated)
    {
        // 地图没有被修改，则从上一帧开始利用IMU观测推算当前帧的位姿
...
void Tracking::Track()
{
    ...
    // 拿到当前Map， 如果Map集合mpAtlas中没有Map则立马新建一个并返回
    Map* pCurrentMap = mpAtlas->GetCurrentMap();
    // 状态：已经有相机帧。根据相机帧时间戳判断视觉数据的正确性,并作出相应处理
    if(mState!=NO_IMAGES_YET)
    {
        if(mLastFrame.mTimeStamp > mCurrentFrame.mTimeStamp)
        {
            // 时间戳信息错误则删除当前IMU观测，并新建一个Map后return（后续开始新的跟踪，建新的Map）
        ...
        {
            // 相机时间戳存在跳跃，相机的频率>10, 时间间隔应<0.01s， 间隔>1.0s，可能丢失数据。。故需要重置当前Map或建立新的Map
            cout << "id last: " << mLastFrame.mnId << "    id curr: " << mCurrentFrame.mnId << endl;
            if(mpAtlas->isInertial()) // 具有惯性观测...
            {
                if(mpAtlas->isImuInitialized()) // IMU已经初始化
                {
                    ...
                    {
                        // 没有做VI-BA,则清空重置当前Map，就不用新建了
                        mpSystem->ResetActiveMap();
                    }
                    else
                    {
                        // 已经做了VI-BA，则保存当前Map并新建一个Map
                        CreateMapInAtlas();
                    }
                }
                else
    ...
    // 当前状态：无相机帧（刚开始Tracking...）
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO) && mpLastKeyFrame)
        mCurrentFrame.SetNewBias(mpLastKeyFrame->GetImuBias()); // 上一KF的IMU bias会传递下去
    ...
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO) && !mbCreatedMap)
    {
...
#endif
        // 只要有IMU观测，每次跟踪进来都会执行PreintegrateIMU()函数，后边的(!mbCreatedMap)条件始终成立（原因见后‘mbCreatedMap = false‘分析。。）
        PreintegrateIMU(); // 执行IMU预积分， 计算雅克比JPa,JPg, 预计分量dP, dR, dV， 协方差矩阵C等信息
#ifdef REGISTER_TIMES
        ...
        vdIMUInteg_ms.push_back(timePreImu); // 保存执行预积分的时间
#endif
    }
    mbCreatedMap = false; // mbCreatedMap的初值是false， 新建Map后mbCreatedMap为true, 但每次跟踪进来mbCreatedMap设为false（好乱的逻辑！）
    ...
    if(mState==NOT_INITIALIZED) // 如果还未初始化，则执行初始化，这里执行的是仅视觉（Only-Vision）的初始化
    {
        if(mSensor==System::STEREO || mSensor==System::RGBD || mSensor==System::IMU_STEREO)
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            // State OK， Local Mapping is activated. This is the normal behaviour,
            // unless you explicitly activate the "only tracking" mode.
            if(mState==OK)
            {
                ...
                if (!bOK) // 如果本次跟踪不成功，更新跟踪状态mState，分为RECENTLY_LOST和完全跟丢LOST
                {
                    if (mCurrentFrame.mnId<=(mnLastRelocFrameId+mnFramesToResetIMU) // 满足该条件，说明相机帧跟踪失败过于频繁
                    && (mSensor==System::IMU_MONOCULAR || mSensor==System::IMU_STEREO))
                    {
                        mState = LOST; // 有IMU观测相机帧跟踪失败还这么频繁，又跟踪失败，则LOST
                    }
            ...
            else // 如果上一帧跟踪不成功，则根据具体情况分别处理，重定位跟丢则利用IMU观测推算相对位姿或再采用基于视觉的重定位，完全丢失（LOST）则新建一个Map
            {
                if (mState == RECENTLY_LOST) // 是RECENTLY_LOST类型跟丢，下一帧继续尝试跟踪？
                {
                ...
                else if (mState == LOST)
                {
                    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO))
                    {
                        if(pCurrentMap->isImuInitialized())
                            PredictStateIMU(); // 如果有IMU观测且惯性观测已初始化，则使用IMU观测计算当前帧的姿态
                        else
                            bOK = false;

                        if (mCurrentFrame.mTimeStamp-mTimeStampLost>time_recently_lost) // time_recently_lost = 5.0
                        {
                            // 已经连续5s跟踪失败，则认为跟踪彻底失败， 也就是说，即使采用了IMU，如果视觉跟踪超过5s也认为跟踪彻底失败
                            // 为何不用IMU一致跟踪下去？没意义啊，继续跟踪下去是仅惯性观测，就成惯性导航了，不能建图，失去了SLAM的意义。
                            mState = LOST;
                            Verbose::PrintMess("Track Lost...", Verbose::VERBOSITY_NORMAL);
                            bOK=false;
                        }
                    }
                    else // 采用基于视觉的重定位（上一帧重定位失败，再利用当前帧尝试重定位）
                    {
                        // TODO fix relocalization
                        bOK = Relocalization();
                        if(!bOK && mCurrentFrame.mTimeStamp-mTimeStampLost>time_recently_lost_visual)
                        {
                            // 重定位失败且视觉跟踪超时，则认为跟踪彻底失败
                            mState = LOST;
                            Verbose::PrintMess("Track Lost...", Verbose::VERBOSITY_NORMAL);
                            bOK=false;
                        }
                    }
                    // 个人认为，这里的处理有些不符合逻辑，如果前边采用视觉方法重定位成功,也要重置或者新建Map吗？
                    // 视觉重定位失败的话毫无疑问，重置或新建Map， Qes: 如果用IMU观测计算相对位姿成功，也要Reset地图吗？
                    Verbose::PrintMess("A new map is started...", Verbose::VERBOSITY_NORMAL);
                    if (pCurrentMap->KeyFramesInMap()<10)
                    {
                        mpSystem->ResetActiveMap(); // 如果当前Map中的KF少于10帧，则直接清空当前Map，不用新建Map
                    ...
                    /*
                     * 退出跟踪了... Qes： 是不是应该判断以下，如果根据惯性推算成功或者重定位成功是不是就别return???
                     * 这里明显有Bug, 后边需要做实验验证，并尝试修改此处的处理逻辑！！！
                     * */
                    return;
                }
            }
        ...
        vdPosePred_ms.push_back(timePosePred); // 保存估计位姿消耗的时间
#endif
        ...
        if(!mbOnlyTracking)
        {
            if(bOK)
            {
                bOK = TrackLocalMap(); // 为当前帧建立更多的3D-2D映射后，采用Only-Vision或Vision-Inertial的方法进行优化当前帧的位姿
            }
            if(!bOK)
        ...
        else
        {
            ...
            // a local map and therefore we do not perform TrackLocalMap(). Once the system relocalizes the camera we will use the local map again.
            if(bOK && !mbVO)
                bOK = TrackLocalMap();
        }
        ...
        else if (mState == OK)
        {
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO)
            {
                ...
                {
                    // 如果局部跟踪不成功，IMU还未初始化或未执行VI-BA(IMU初始化时会调用VI-BA)， 则重置Map
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            if(!mLastFrame.mTcw.empty() && !mCurrentFrame.mTcw.empty())
            {
                ...
                mVelocity = mCurrentFrame.mTcw*LastTwc; // 实质是上一帧和当前帧的相对位姿，即上一帧到当前帧的位姿变换矩阵
            }
            else
                mVelocity = cv::Mat();
...
#endif
            bool bNeedKF = NeedNewKeyFrame(); // 关键帧判断，判断是否需要插入一个新的关键帧
            // Check if we need to insert a new keyframeTo do so,
            if(bNeedKF && (bOK|| (mState==RECENTLY_LOST && (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO))))
                CreateNewKeyFrame(); // 用当前帧的信息new一个关键帧， 然后将new的KF插入到LocalMapping线程
    ...
    if(mState==OK || mState==RECENTLY_LOST)
    {
        ...
        if(!mCurrentFrame.mTcw.empty())
        {
            cv::Mat Tcr = mCurrentFrame.mTcw*mCurrentFrame.mpReferenceKF->GetPoseInverse(); // 参考KF到当前帧的位姿变换
            mlRelativeFramePoses.push_back(Tcr);
            mlpReferences.push_back(mCurrentFrame.mpReferenceKF); // 在参考关键帧数组中加入新的KF
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    mpLocalMapper->InsertKeyFrame(pKFini); // 插入新的关键帧
...
/*
 * @brief 在Atlas中新建一个Map， 并设置初始化类对象和预积分类对象
 * */
void Tracking::CreateMapInAtlas()
{
    mnLastInitFrameId = mCurrentFrame.mnId;
    mpAtlas->CreateNewMap(); // 在Atlas中新建一个Map， 所有Map是在Atlas中存储和管理
    if (mSensor==System::IMU_STEREO || mSensor == System::IMU_MONOCULAR)
        mpAtlas->SetInertialSensor(); // 标记：存在惯性观测
    mbSetInit = false;

    mnInitialFrameId = mCurrentFrame.mnId+1; // new Map从id为mnInitialFrameId的KF开始？
    mState = NO_IMAGES_YET;

    // Restart the variable with information about the last KF
    mVelocity = cv::Mat(); // 新建Map时速度模型为空
    ...
    {
        delete mpImuPreintegratedFromLastKF; // 删除上一KF预积分的结果
        mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),*mpImuCalib); // 预积分的初始化步骤
    }
...
bool Tracking::TrackWithMotionModel()
{
    ...
    && (mCurrentFrame.mnId>mnLastRelocFrameId+mnFramesToResetIMU)) // 这个条件意思：过去超过mnFramesToResetIMU帧跟踪状态稳定
    {
        // Predict ste with IMU if it is initialized and it doesnt need reset
        // IMU已初始化，跟踪状态稳定，则采用IMU观测推算当前帧的位姿
        PredictStateIMU();
        return true;
    }
    else
...
/*
 * @brief 跟踪局部Map，主要为了优化当前帧的位姿
 * 过程：更新局部KF数组；更新局部Map点；基于3D-2D对应关系优化当前帧的位姿（包括仅视觉的优化和基于视觉-惯性观测的优化）
 * */
bool Tracking::TrackLocalMap()
{
...
#endif
    UpdateLocalMap(); // 更新局部关键帧数组LocalKeyFrames和被LocalKeyFrames看到的Map点
#ifdef REGISTER_TIMES
    ...
    SearchLocalPoints(); // 在mvpLocalMapPoints数组中搜索被当前帧可见的Map点，并与特征点建立关联
#ifdef REGISTER_TIMES
    ...
    int inliers;
    if (!mpAtlas->isImuInitialized())
        Optimizer::PoseOptimization(&mCurrentFrame); // 仅视觉的优化更新当前帧的位姿
    else
    {
        if(mCurrentFrame.mnId<=mnLastRelocFrameId+mnFramesToResetIMU) // 用于验证是否长时间稳定跟踪
        {
            // 如果稳定跟踪时间较短则采用Only-Vision的优化
        ...
        else
        {
            if(!mbMapUpdated)
            {
                ...
                // 基于Vision-Inertial观测的优化
                inliers = Optimizer::PoseInertialOptimizationLastFrame(&mCurrentFrame); // , !mpLastKeyFrame->GetMap()->GetIniertialBA1());
            }
            else
            {
                // 如果地图已被更新，则以最近kF为基准进行优化
    ...
    // Monocular, Stereo在每帧上跟踪-优化后，内点数需>30, 加IMU观测时内点数需>15
    if (mSensor == System::IMU_MONOCULAR)
    {
...
void Tracking::CreateNewKeyFrame()
{
    ...
    mpLocalMapper->InsertKeyFrame(pKF); // 在LocalMapper线程中插入一个新的关键帧
...
/*
 * @brief mvpLocalMapPoints数组中通过投影-匹配的策略搜索被当前帧可见的Map点，并与特征点建立关联
 * */
void Tracking::SearchLocalPoints()
{
    ...
    // 检查mvpLocalMapPoints中被当前帧可视的Map点
    for(vector<MapPoint*>::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        if(mCurrentFrame.isInFrustum(pMP,0.5)) // 通过将Map点投影到影像上，判断Map点是否在相机帧的视锥体
        {
            pMP->IncreaseVisible();
            nToMatch++;
        }
...
/*
 * @brief LocalKF组数可见且不是当前帧跟踪得到的Map点加入到LocalMap数组
 */
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO) &&mvpLocalKeyFrames.size()<80)
    {
        ...
        for(int i=0; i<Nd; i++)
        {
            ...
            if(tempKeyFrame->mnTrackReferenceForFrame!=mCurrentFrame.mnId)
            {
                ...
                tempKeyFrame->mnTrackReferenceForFrame=mCurrentFrame.mnId; // 不断的添加PreKF到mvpLocalKeyFrames数组
    ...
    if(pKFmax)
    {
        mpReferenceKF = pKFmax; // 具有最多共识的KF为参考KF
        mCurrentFrame.mpReferenceKF = mpReferenceKF;
    }
}
...
bool Tracking::Relocalization()
{
    ...
    // 根据DBoW相似性在当前Map的关键帧数据库中寻找重定位候选帧
    ...
    vector<MLPnPsolver*> vpMLPnPsolvers; // 与ORB-SLAM2不同的是这块采用MLPnP由3D-2D映射计算位姿
...
/*
 * @brief 更新上一帧和当前帧的位姿及IMU的P, R， V， biases
 * */
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    ...
    for(list<cv::Mat>::iterator lit=mlRelativeFramePoses.begin(),lend=mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lbL++)
    {
        ...
        if(pKF->GetMap() == pMap)
        {
            (*lit).rowRange(0,3).col(3)=(*lit).rowRange(0,3).col(3)*s; // 帧间相对位姿的平移量乘scale， 注意：R不乘scale是因为后边变换时会考虑。
        }
    }

    // 更新Tracking线程中mLastFrame和mCurrentFrame的IMU偏置
    mLastBias = b;
    ...
    // 用IMU观测推算位姿是基于LastFrame的位姿，并利用LastFrame到CurrentFrame间的惯性观测计算当前帧的位姿，所以要更新LastFrame的时IMU的P, R, V, biases（更新基准）
    if(mLastFrame.mnId == mLastFrame.mpLastKeyFrame->mnFrameId) // mLastFrame就是一个关键帧
    {
        // 设置mLastFrame时IMU的P, R, V, biases和mLastFrame的位姿
