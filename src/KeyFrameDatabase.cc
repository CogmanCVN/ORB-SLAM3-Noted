...
/*
 * @brief 获取闭环探测的候选KF和Map合并的候选KF
 * 过程：1）在关键帧数据库搜索与查询KF具有相同word的KF的数组lKFsSharingWords
 * 2) 设定最小共用word的个数阈值minCommonWords=maxCommonWords*0.8;
 * 3) 计算lKFsSharingWords中共用word大于minCommonWords的KF与查询KF的BoW相似性得分得到lScoreAndMatch数组
 * 4）lScoreAndMatch中的KFi及其10个最佳共视帧最高得分的KF被选中作为闭环和Map合并的候选帧
 * 5）根据是否与查询帧在同一个Map上，去选定闭环候选KF和Map合并候选KF
 * */
void KeyFrameDatabase::DetectNBestCandidates(KeyFrame *pKF, vector<KeyFrame*> &vpLoopCand, vector<KeyFrame*> &vpMergeCand, int nNumCandidates)
{
    ...
    // Search all keyframes that share a word with current frame
    {
        ...
        spConnectedKF = pKF->GetConnectedKeyFrames(); // 根据共视图获取与pKF相互共视的KF的集合
        ...
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
        {
            ...
            for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
            {
                ...
                if(pKFi->mnPlaceRecognitionQuery!=pKF->mnId)
                {
                    ...
                    {
                        pKFi->mnPlaceRecognitionQuery=pKF->mnId;
                        lKFsSharingWords.push_back(pKFi); // 保存KF数据库中与pKF(当前帧)具有相同words但非共视的KF
                    }
                }
                pKFi->mnPlaceRecognitionWords++;
            }
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        ...
        {
            nscores++;
            float si = mpVoc->score(pKF->mBowVec,pKFi->mBowVec); // 当前帧和候选帧的BoW视觉相似性的分
            pKFi->mPlaceRecognitionScore=si;
            lScoreAndMatch.push_back(make_pair(si,pKFi));
        }
    }
    ...
    {
        KeyFrame* pKFi = it->second;
        vector<KeyFrame*> vpNeighs = pKFi->GetBestCovisibilityKeyFrames(10); // 获取与pKFi最佳共视的10个关键帧

        float bestScore = it->first; // pKFi及其共视帧与当前帧（查询帧）最高的BoW视觉相似性得分
        float accScore = bestScore; // 如果pKFi及其共视帧与当前帧（查询帧）相似性得分的求和结果
        KeyFrame* pBestKF = pKFi;
        for(vector<KeyFrame*>::iterator vit=vpNeighs.begin(), vend=vpNeighs.end(); vit!=vend; vit++)
        {
            KeyFrame* pKF2 = *vit;
            if(pKF2->mnPlaceRecognitionQuery!=pKF->mnId)
                continue; // 候选帧的邻近共视帧必须同样也是闭环候选帧
        ...
        lAccScoreAndMatch.push_back(make_pair(accScore,pBestKF)); // make_pair(累计得分，具有最高相似性得分的候选镇)
        if(accScore>bestAccScore)
            bestAccScore=accScore;
    }

    lAccScoreAndMatch.sort(compFirst); // 降序排序
    ...
    while(i < lAccScoreAndMatch.size() && (vpLoopCand.size() < nNumCandidates || vpMergeCand.size() < nNumCandidates))
    {
        ...
        if(!spAlreadyAddedKF.count(pKFi))
        {
            // 查询帧pKF和候选帧pKFi的当前所属的Map是相同的，则pKFi是闭环候选帧，否则是Map合并的候选帧
            if(pKF->GetMap() == pKFi->GetMap() && vpLoopCand.size() < nNumCandidates)
            {
                vpLoopCand.push_back(pKFi); // 加入闭环探测的候选帧数组
            }
            else if(pKF->GetMap() != pKFi->GetMap() && vpMergeCand.size() < nNumCandidates && !pKFi->GetMap()->IsBad())
            {
                vpMergeCand.push_back(pKFi); // 加入Map合并候选帧数组
            }
            spAlreadyAddedKF.insert(pKFi); // 用set集合做标记，pKFi已经被选中
        }
        i++;
        it++;
    }
