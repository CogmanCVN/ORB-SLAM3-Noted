...
{
    // Load IMU pose
    twb = Converter::toVector3d(pKF->GetImuPosition()); // 体坐标系到世界坐标系的t
    Rwb = Converter::toMatrix3d(pKF->GetImuRotation()); // 体坐标系到世界坐标系的R
...
ImuCamPose::ImuCamPose(Frame *pF):its(0)
{
    ...
    // For posegraph 4DoF // IMU的VP设为顶点参与优化时，g2o优化的是这两项？
    Rwb0 = Rwb;
    DR.setIdentity();
}
...
void EdgeMono::linearizeOplus()
{
    ...
    const Eigen::Vector3d Xb = VPose->estimate().Rbc[cam_idx]*Xc+VPose->estimate().tbc[cam_idx]; // 3D点转到体坐标系下
    ...
    _jacobianOplusXi = -proj_jac * Rcw; // proj_jac在世界坐标系，需要转到相机坐标系
    ...
    _jacobianOplusXj = proj_jac * Rcb * SE3deriv; // 在世界坐标系？ // TODO optimize this product
}
    ...
    JPa(Converter::toMatrix3d(pInt->JPa)), mpInt(pInt), dt(pInt->dT)
{
    ...
    // 由协方差矩阵C计算信息矩阵Info, Qes: 参考的公式还未找到？？？
    resize(6); // 设定_jacobianOplus和_hessian数组的大小
    g << 0, 0, -IMU::GRAVITY_VALUE;
    cv::Mat cvInfo = pInt->C.rowRange(0,9).colRange(0,9).inv(cv::DECOMP_SVD);
    Matrix9d Info;  // 信息矩阵，协方差的逆矩阵
    for(int r=0;r<9;r++)
        for(int c=0;c<9;c++)
            Info(r,c)=cvInfo.at<float>(r,c);
    Info = (Info+Info.transpose())/2; // (Info + Info_T) * 0.5
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double,9,9> > es(Info); // SelfAdjointEigenSolver用于计算Info矩阵的特征值和特征向量
    Eigen::Matrix<double,9,1> eigs = es.eigenvalues();
     for(int i=0;i<9;i++)
         if(eigs[i]<1e-12)
             eigs[i]=0;
    Info = es.eigenvectors()*eigs.asDiagonal()*es.eigenvectors().transpose(); // eigs.asDiagonal()是用特征值构造一个对称矩阵 A = VEV_T
    setInformation(Info);
}
...
void EdgeInertial::computeError()
{
    ...
    // 参考forster论文 附录部分公式（A.21）
    const VertexPose* VP1 = static_cast<const VertexPose*>(_vertices[0]); // 6-dof, 3个线元素，3个角元素
...
void EdgeInertial::linearizeOplus()
{
    ...
    // 以下参见forster论文补充材料 ‘Supplementary Material to: IMU Preintegration on Manifold Efficient Visual-Inertial ...’
    // Jacobians wrt Pose 1
    _jacobianOplus[0].setZero();
     // rotation // R, V ,P对phi的偏导
    _jacobianOplus[0].block<3,3>(0,0) = -invJr*Rwb2.transpose()*Rwb1;                           // 参见forster论文 Page.8 r_△R对phi_i的偏导
    _jacobianOplus[0].block<3,3>(3,0) = Skew(Rbw1*(VV2->estimate() - VV1->estimate() - g*dt));  // 参见Page.7 r_△v对phi_i的偏导
    _jacobianOplus[0].block<3,3>(6,0) = Skew(Rbw1*(VP2->estimate().twb - VP1->estimate().twb
                                                   - VV1->estimate()*dt - 0.5*g*dt*dt));        // 参见Page.6 r_△p对phi_i的偏导
    // translation
    _jacobianOplus[0].block<3,3>(6,3) = -Eigen::Matrix3d::Identity(); // 参见Page.6 r_△p对p_i的偏导

    // Jacobians wrt Velocity 1 // R, V ,P对v_i的偏导
    _jacobianOplus[1].setZero();                     // 参见Page.8 r_△R对v_i的偏导
    _jacobianOplus[1].block<3,3>(3,0) = -Rbw1;       // 参见Page.7 r_△v对v_i的偏导
    _jacobianOplus[1].block<3,3>(6,0) = -Rbw1*dt;    // 参见Page.6 r_△p对v_i的偏导

    // Jacobians wrt Gyro 1
    _jacobianOplus[2].setZero();
    _jacobianOplus[2].block<3,3>(0,0) = -invJr*eR.transpose()*RightJacobianSO3(JRg*dbg)*JRg; // 参见Page.8 r_△R对b_g的偏导
    _jacobianOplus[2].block<3,3>(3,0) = -JVg;   // 参见Page.7 r_△v对b_g的偏导
    _jacobianOplus[2].block<3,3>(6,0) = -JPg;   // 参见Page.6 r_△p对b_g的偏导

    // Jacobians wrt Accelerometer 1
    _jacobianOplus[3].setZero();                // 参见Page.8 r_△R对b_a的偏导
    _jacobianOplus[3].block<3,3>(3,0) = -JVa;   // 参见Page.7 r_△v对b_a的偏导
    _jacobianOplus[3].block<3,3>(6,0) = -JPa;   // 参见Page.6 r_△p对b_a的偏导

    // Jacobians wrt Pose 2
    _jacobianOplus[4].setZero();
    // rotation
    _jacobianOplus[4].block<3,3>(0,0) = invJr;     // 参见Page.8 r_△R对phi_j的偏导
    // translation
    _jacobianOplus[4].block<3,3>(6,3) = Rbw1*Rwb2; // 参见Page.6 r_△p对p_j的偏导

    // Jacobians wrt Velocity 2
    _jacobianOplus[5].setZero();
    _jacobianOplus[5].block<3,3>(3,0) = Rbw1;      // 参见Page.7 r_△v对v_j的偏导
}
...
void EdgeInertialGS::computeError()
{
    ...
    // 计算残差顾忌了尺度
    const Eigen::Vector3d er = LogSO3(dR.transpose()*VP1->estimate().Rwb.transpose()*VP2->estimate().Rwb);
    const Eigen::Vector3d ev = VP1->estimate().Rwb.transpose()*(s*(VV2->estimate() - VV1->estimate()) - g*dt) - dV; // 速度和pose误差顾忌了尺度
...
void EdgeInertialGS::linearizeOplus()
{
    ...
    // Jacobians wrt scale factor    尺度的雅克比
...
/*
 * @brief 计算R, P, V, bg, ba的残差， Qes: 对照公式？？？
 * */
