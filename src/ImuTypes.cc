...
/*
 * 【写在之前】
 * IMU预积分部分的理论和公式等可参考forster的论文“IMU Preintegration on Manifold for Efficient Visual-Inertial Maximum-a-Posteriori Estimation”
 * 另外还有其补充文献"Supplementary Material to: IMU Preintegration on Manifold for Efficient Visual-Inertial Maximum-a-Posteriori Estimation"
 * 搞懂IMU预积分，强烈推荐阅读高翔博士的知乎文章：简明预积分推导， https://zhuanlan.zhihu.com/p/388859808
 * 致谢：感谢知乎博主https://www.zhihu.com/people/mi-mi-mi-31-24 的分享
 * */
...
const float eps = 1e-4; // 小于eps则认为是小量
    ...
    deltaT(time)
{
    ...
    if(d<eps)
    {
        // 旋转比较小，旋转向量到旋转矩阵的指数映射采用一阶近似
        deltaR = I + W; // 参见forster论文公式(4) exp(phi^) = I + phi^
        rightJ = cv::Mat::eye(3,3,CV_32F); // 小量时，，右扰动 Jr = I
    }
    else
    {
        deltaR = I + W*sin(d)/d + W*W*(1.0f-cos(d))/d2; // forster经典预积分论文公式(3)
...
void Preintegrated::Initialize(const Bias &b_)
{
    ...
    C = cv::Mat::zeros(15,15,CV_32F); // 协方差传递所需的A,B矩阵，可参考forster论文中附录部分的公式(A.9)
    Info = cv::Mat();
...
void Preintegrated::IntegrateNewMeasurement(const cv::Point3f &acceleration, const cv::Point3f &angVel, const float &dt)
{
    ...
    // 首先更新Position，因为它取决于先前计算的速度和旋转，其次更新Velocity，它取决于先前计算的旋转，Rotation是最后被更新
    //Matrices to compute covariance
    // 计算协方差时的A, B矩阵， 以下计算可参考forster论文中附录部分的公式(A.7~A.9)
    cv::Mat A = cv::Mat::eye(9,9,CV_32F);
    cv::Mat B = cv::Mat::zeros(9,6,CV_32F);

    // 加速度和角速度还需要考虑随机游走
    cv::Mat acc = (cv::Mat_<float>(3,1) << acceleration.x-b.bax,acceleration.y-b.bay, acceleration.z-b.baz); // (a_k - b_a)
    cv::Mat accW = (cv::Mat_<float>(3,1) << angVel.x-b.bwx, angVel.y-b.bwy, angVel.z-b.bwz); // (w_k - b_w)

    // 计算平均加速度和角速度向量
    avgA = (dT*avgA + dR*acc*dt)/(dT+dt); // acc*dt是速度增量
    avgW = (dT*avgW + accW*dt)/(dT+dt); // accW*dt是旋转角度增量

    // Update delta position dP and velocity dV (rely on no-updated delta rotation)
    // 更新P, V的预积分量， 可参见forster论文公式（26）
    ...
    cv::Mat Wacc = (cv::Mat_<float>(3,3) << 0, -acc.at<float>(2), acc.at<float>(1),
                                                   ...
                                                   -acc.at<float>(1), acc.at<float>(0), 0);    // Wacc是由加速度向量得到的反对称矩阵(a_k - b_a)^
    // 参见forster论文附录公式(A.7)
    ...
    // 预积分量对b_a和b_g变化量的雅克比，ORB-SLAM3对forster论文附录公式(A.20)做了变形，采用迭代更新,详见 https://github.com/UZ-SLAMLab/ORB_SLAM3/issues/212
    JPa = JPa + JVa*dt -0.5f*dR*dt*dt;
    JPg = JPg + JVg*dt -0.5f*dR*dt*dt*Wacc*JRg;
    JVa = JVa - dR*dt;
    JVg = JVg - dR*dt*Wacc*JRg;

    // Update delta rotation, dR， 旋转最后更新
    IntegratedRotation dRi(angVel,b,dt);
    dR = NormalizeRotation(dR * dRi.deltaR); // 更新旋转积分：i到k的旋转积分dR * (k, k+1)区间的旋转积分deltaR

    // Compute rotation parts of matrices A and B, 参见forster论文附录公式(A.7)
    A.rowRange(0,3).colRange(0,3) = dRi.deltaR.t();
    B.rowRange(0,3).colRange(0,3) = dRi.rightJ*dt;

    // Update covariance 参见forster论文附录公式(A.9)
    C.rowRange(0,9).colRange(0,9) = A*C.rowRange(0,9).colRange(0,9)*A.t() + B*Nga*B.t(); // C(0~9) = A*C(0~9)*A.t() + B*Nga*B.t()
    C.rowRange(9,15).colRange(9,15) = C.rowRange(9,15).colRange(9,15) + NgaWalk; // C(9~15) = C(9~15) + NgaWalk, NgaWalk size is 6x6
...
{
    std::unique_lock<std::mutex> lock(mMutex);
    cv::Mat dbg = (cv::Mat_<float>(3,1) << b_.bwx-b.bwx,b_.bwy-b.bwy,b_.bwz-b.bwz); // 更新bias, 详细参考forster论文 C. Incorporating Bias Updates小节
    return NormalizeRotation(dR*ExpSO3(JRg*dbg)); // 参考forster论文式（36）
}
...
cv::Mat Preintegrated::GetDeltaVelocity(const Bias &b_)
{
    ...
    return dV + JVg*dbg + JVa*dba; // 参考forster论文式（36）
}

cv::Mat Preintegrated::GetDeltaPosition(const Bias &b_)
{
    std::unique_lock<std::mutex> lock(mMutex);
    // 加速度偏置和角速度偏置均会对位置计算产生影响
    cv::Mat dbg = (cv::Mat_<float>(3,1) << b_.bwx-b.bwx,b_.bwy-b.bwy,b_.bwz-b.bwz);
    cv::Mat dba = (cv::Mat_<float>(3,1) << b_.bax-b.bax,b_.bay-b.bay,b_.baz-b.baz);
    return dP + JPg*dbg + JPa*dba; // 参考forster论文式（36）
}
