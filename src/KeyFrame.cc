...
/*
 * @brief 返回体坐标系到世界坐标系的旋转矩阵
 * */
cv::Mat KeyFrame::GetImuRotation()
{
    unique_lock<mutex> lock(mMutexPose);
    return Twc.rowRange(0,3).colRange(0,3)*mImuCalib.Tcb.rowRange(0,3).colRange(0,3); // 体坐标系到相机坐标系的变换提前采用标定得到
}
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    return mvpMapPoints; // 返回通过ORB匹配关联到的Map点
}
...
/*
 * @brief 更新该帧与其他帧的连接信息
 * 检查该帧关联的每个Map点的所有obsKF, 如果obsKF未与该KF建立连接，则建立连接关系
 * */
