...
public:
    ...
    Bias(const float &b_acc_x, const float &b_acc_y, const float &b_acc_z,     // 注意，该构造函数的顺序是ba, bg;
...
public:
    cv::Mat Tcb; // IMU to camera // 可以发现，IMU算的旋转是从体坐标系到相机坐标系，而非体坐标系到世界坐标，而相机帧会跟踪得到相机坐标系到世界坐标系的旋转
    cv::Mat Tbc; // camera to IMU(body frame) 4x4变换矩阵
    cv::Mat Cov, CovWalk; // x, y, z三个方向上(所以Cov和CovWalk均是6x6的矩阵)的噪声和随机游走的协方差矩阵
};
...
public:
    ...
    cv::Mat deltaR; // integrated rotation 当前IMU观测计算出的旋转预积分△R_k,k+1， 注意和旋转积极dR,△R_ik区分
    cv::Mat rightJ; // right jacobian 雅克比右扰动
};
...
public:
    float dT; // 相机帧间IMU测量时间△t_i,j, 时间越长IMU测量的误差越大(随时间存在累积效应) 另外，采样频率>100Hz的IMU才能满足无人驾驶的需求
    cv::Mat C; // 协方差矩阵C= A*C*A.t() + B*Nga*B.t(); C(9~12)是bg在x, y, z三方向的协方差矩阵（求逆得到信息矩阵）；C(12~15)是ba在x, y, z三方向的协方差矩阵
    cv::Mat Info; // 信息矩阵
    cv::Mat Nga, NgaWalk; // Nga是噪声项ηa, ηb在x,y,z三方向的协方差矩阵；NgaWalk是随机游走bg, ba在三方向的协方差矩阵，两矩阵的size: 6x6

    // Values for the original bias (when integration was computed)
    Bias b;
    cv::Mat dR, dV, dP; // dR是旋转预积分△R_ik，初值是单位阵； dV是速度预积分△V_ik，3x1，初值为0； dP是位置预积分△P_ik，3x1，初值是0
    cv::Mat JRg, JVg, JVa; // JRg是旋转预积分对delta(b_g)的雅克比； JVg是速度预积分对delta(b_g)的雅克比；JVa是速度预积分对delta(b_a)的雅克比；
    cv::Mat JPg, JPa; // JPg是位置预积分对delta(b_g)的雅克比； JPa是位置预积分对delta(b_a)的雅克比
    cv::Mat avgA; // 平均加速度，用于判断加速度是否发生变化
    cv::Mat avgW; // 平均角速度，用于判断角速度是否发生变化
...
private:
    ...
    std::vector<integrable> mvMeasurements; // IMU积分观测的数组， a和w已经采用mid-point方法平均化
    std::mutex mMutex;
};
