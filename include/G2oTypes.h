...
public:
    ...
    Eigen::Vector2d Project(const Eigen::Vector3d &Xw, int cam_idx=0) const; // Mono  Project函数中用到了Rcw和tcw
...
public:
    // For IMU
    Eigen::Matrix3d Rwb; // IMU的位姿
    Eigen::Vector3d twb;

    // For set of cameras
    std::vector<Eigen::Matrix3d> Rcw; // R: world to camera // 在优化时VP->estimate().Rcw[0] 得到相机帧的位姿的旋转矩阵， 即Rcw和tcw参与VI-BA的优化
    std::vector<Eigen::Vector3d> tcw; // t: world to camera // 在优化时VP->estimate().tcw[0] 得到相机帧位姿的线元素
    std::vector<Eigen::Matrix3d> Rcb, Rbc; // Rcb: body frame to camera rotation
    std::vector<Eigen::Vector3d> tcb, tbc; // tcb: body frame to camera translation
    double bf;
...
public:
    ...
    virtual void oplusImpl(const double* update_){
        double update6DoF[6];
        update6DoF[0] = 0; // roll = 0
        update6DoF[1] = 0; // pitch = 0
        update6DoF[2] = update_[0]; // yaw 参与优化
...
public:
    ...
    void computeError(){
        ...
        const VertexPose* VPose = static_cast<const VertexPose*>(_vertices[1]); //Qes: VPose是怎么从VertexPose中得到相机帧的位姿的？？？
...
public:
    ...
    // _jacobianOplus[0]是△r, △v, △p对phi_i的偏导, △p对p_i的偏导； _jacobianOplus[1]是△r, △v, △p对v_i的偏导； _jacobianOplus[2]是△r, △v, △p对bg的偏导
    // _jacobianOplus[3]是△r, △v, △p对ba的偏导； _jacobianOplus[4]是△r对phi_j, △p对p_j的偏导； _jacobianOplus[5]是V对v_j的偏导
    Eigen::Matrix<double,24,24> GetHessian(){
        linearizeOplus(); // 计算雅克比
        Eigen::Matrix<double,9,24> J;
        J.block<9,6>(0,0) = _jacobianOplus[0]; // 修改从(0, 0)开始，大小为9行6列的J矩阵块的值
        J.block<9,3>(0,6) = _jacobianOplus[1];
        J.block<9,3>(0,9) = _jacobianOplus[2];
        J.block<9,3>(0,12) = _jacobianOplus[3];
        J.block<9,6>(0,15) = _jacobianOplus[4];
        J.block<9,3>(0,21) = _jacobianOplus[5];
        return J.transpose()*information()*J; // 由一阶雅克比计算Hessian矩阵： H = J_T * Info * J(H是24 X 24的矩阵)
    }
...
public:
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW
        EdgePriorPoseImu(ConstraintPoseImu* c); // 给出Hessian矩阵

        virtual bool read(std::istream& is){return false;}
        virtual bool write(std::ostream& os) const{return false;}

        void computeError();   // 计算R, P, V, bg, ba的残差
        virtual void linearizeOplus(); // 线性化计算雅克比
...
public:
    ...
    void computeError(){
        ...
        _error << LogSO3(VPi->estimate().Rcw[0]*VPj->estimate().Rcw[0].transpose()*dRij.transpose()), // 旋转向量残差
                 VPi->estimate().Rcw[0]*(-VPj->estimate().Rcw[0].transpose()*VPj->estimate().tcw[0])+VPi->estimate().tcw[0] - dtij; // 平移量残差
    }
