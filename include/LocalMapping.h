...
public:
    ...
    Eigen::Matrix3d mRwg; // mRwg用于计算重力在世界坐标系中的方向g的旋转矩阵， g = mRwg * g_I;
    Eigen::Vector3d mbg;
    Eigen::Vector3d mba;
    double mScale;
    double mInitTime;
    ...
    bool mbNotBA1;
    bool mbNotBA2;
    bool mbBadImu; // IMU观测是否可用， 构造函数中默认设置为false
...
protected:
    ...
    std::list<MapPoint*> mlpRecentAddedMapPoints; // 新插入的Map点

    std::mutex mMutexNewKFs;

    bool mbAbortBA; // 是否停止BA, true: 停止BA
    ...
    bool mbAcceptKeyFrames; // false： 不接受新插入KF
    std::mutex mMutexAccept;

    void InitializeIMU(float priorG = 1e2, float priorA = 1e6, bool bFirst = false);
    void ScaleRefinement();

    bool bInitializing; // 是否正在执行IMU初始化， true: 是； false: 否
