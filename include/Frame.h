...
public:
    ...
    ConstraintPoseImu* mpcpi; // 由IMU观测做优化后计算出来的体坐标系到世界坐标的位姿

    bool imuIsPreintegrated();
    void setIntegrated();

    cv::Mat mRwc;
    cv::Mat mOw; // 相机中心在世界坐标系中的坐标
public:
    ...
    cv::Mat mPredRwb, mPredtwb, mPredVwb; // 根据IMU观测推算出来的R, P， V
    IMU::Bias mPredBias; //

    // IMU bias
    IMU::Bias mImuBias;

    // Imu calibration, 从配置文件读取得到
    IMU::Calib mImuCalib;

    // Imu preintegration from last keyframe
    IMU::Preintegrated* mpImuPreintegrated; // 从上一帧关键帧开始预积分（FromLastKF）， 里边存储了预积分的雅克比，协方差矩阵等信息
    KeyFrame* mpLastKeyFrame; // 从上一KF开始计算预积分的KF

    // Pointer to previous frame
    Frame* mpPrevFrame;
    IMU::Preintegrated* mpImuPreintegratedFrame; // 从上一邻近帧开始预积分（FromLastFrame），里边存储了预积分的雅克比，协方差矩阵等信息
...
private:
    ...
    cv::Matx31f mOwx; // 相机中心在世界坐标系中的坐标
    cv::Matx33f mRcwx; // world to camera
    cv::Matx31f mtcwx;

    bool mbImuPreintegrated; // 是否已经执行预积分， true: 是（不需要对当前帧做IMU积分，此时可能是第一帧，也可能已经完成预积分）； false: 否（需要执行预积分）
...
public:
    ...
    cv::Mat mTlr, mRlr, mtlr, mTrl; // mTlr： 右相机到左相机的相对位姿变换
